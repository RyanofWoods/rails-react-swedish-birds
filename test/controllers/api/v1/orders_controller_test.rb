...
class Api::V1::OrdersControllerTest < ActionDispatch::IntegrationTest
  setup do
    @order = orders(:woodpeckers_and_alies)
    @user = users(:ryan)
  ...
  test 'GET #show returns the family with its birds and any user observations joined on' do
    ...
    get api_v1_order_url(@order.scientific_name)

    assert_response :success
    expected = {
      'total_birds'=> 3,
      'total_seen'=> 2,
      'group_scientific_name'=> @order.scientific_name,
      'group_english_name'=> @order.english_name,
      'group_swedish_name'=> @order.swedish_name,
      'birds'=> [
        {
