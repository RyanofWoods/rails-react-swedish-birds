...
import { fetchBirdsResponse, createObservationResponse, createObservationRequest, searchBirdsResponse } from '../types/api'
...
export const searchBirds = createAsyncThunk(
  'birds/search',
  async (query: string) => {
    const response = await client.post<searchBirdsResponse>(`/search?query=${query}`, query)
    return response.result
  }
)
